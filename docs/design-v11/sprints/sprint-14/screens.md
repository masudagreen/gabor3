...
> **Sprint 22 v1.2 改訂注記（2026-05-10、最重要）**：本スプリントの G-06 / G-07 は **v1.2 で大幅改訂**された。
> - **G-06**：「左右 2 ガボール直接選択」方式は廃止され、**3×3 グリッド + 違うパッチ複数ランダム + 複数選択 + TP+1/FP-1 部分点**方式に統合（G-04 / G-05 と共通フォーマット）。窓サイズ範囲は v1.1 維持（A3 確定）。
> - **G-07**：4×4 グリッドは維持。メッセージは「向きが同じものを選んで」（個数伏字、A11 確定）、正解アイテム個数ランダム、採点 TP+1/FP-1 部分点（他 3×3 系と統一）。
> - 最新仕様は `docs/design-v11/sprints/sprint-22/screens.md` §5 G-04 / G-05 / G-06 / §6 G-07 を参照。**本書 S14-01〜S14-06 の記述は履歴目的でのみ残置**。新規実装は sprint-22 を参照すること。
>
