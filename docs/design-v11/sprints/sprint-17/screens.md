...
> **Sprint 22 v1.2 改訂注記（2026-05-10、最重要）**：本スプリントの **G-12 は v1.2 でリリース対象外（完全削除）**、**G-13 は v1.2 で領域拡大改訂**された。
> - **G-12**：v1.2 で完全削除（コード・データ・履歴・ナビ・index 全てから除外）。S17-01〜S17-03 の記述は無効化。
> - **G-13**：刺激領域を「**安全領域内で上 70% / キーパッド 30%**」に最大化（A4 確定）。keypad-10（5×2 配置）は維持。最新仕様は `docs/design-v11/sprints/sprint-22/screens.md` §7 G-13 を参照。**本書 S17-04〜S17-06 の記述は履歴目的でのみ残置**。新規実装は sprint-22 を参照すること。
>
