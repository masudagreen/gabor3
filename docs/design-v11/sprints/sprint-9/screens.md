...
> **Sprint 22 v1.2 改訂注記（2026-05-10、最重要）**：本スプリントの G-01 は **v1.2 で完全に再設計**された。「変化察知（角度モーフィング）」コンセプトは廃止され、**「ゆっくり回転検出」コンセプト**に置き換わった。グリッドは 3×3 固定、回転アイテム複数ランダム、両方向回転、回転速度 staircase、複数選択 TP+1/FP-1 部分点、メッセージ「回転しているパッチをタップ」。最新仕様は `docs/design-v11/sprints/sprint-22/screens.md` §3 G-01 を参照。**本書 S9-01 / S9-02 / S9-03 の記述は履歴目的でのみ残置**。新規実装は sprint-22 を参照すること。
>
