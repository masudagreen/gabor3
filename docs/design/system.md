# GaborEye v3.1 — デザインシステム（レベル管理・セッション制）

> **位置づけ**：本書は v3.0（スコア管理 → レベル管理リブート）→ **v3.1（セッション制）** のデザインシステムの **ソース・オブ・トゥルース** です。仕様書 `docs/spec.md`（v3.1、2026-06-10 凍結）に 1 対 1 で対応します。
...
# ゲーム（F-01/F-02/F-12）
game.level = "レベル {n}"
game.count.findN = "{n} 個探せ！"
game.countdown.remaining = "残り {n} 秒"
game.abort.iconLabel = "ゲームを中断"
game.a11y.start = "レベル {level}。{count} 個の回転を探してください"
...
## 15. システムバージョン
- v3.1（本書が最新規範）。`v3.1.x` を設定バージョン情報に表示（F-13）。v3.0 は本書の土台として継承。
