...
> **V3.2 リリース（2026-06-21）— レベルモデル刷新 + チュートリアル**
> 起動・テスト・ビルドコマンドは §V3.2 と不変（`npm test` / `npm run build:web` / `npx serve -s dist -l 4599`）。
> v3.2 変更点（`docs/spec.md` §0.0 が SoT・S18〜S22）：
> - **レベルモデル**：mixed-radix の最内側軸を「個数」→「**繰り返し回数 n**（radix=n、設定 `repeatCount` 1〜6・既定4）」へ置換。総レベル = n×180（既定 720 維持）。`src/lib/v3/level.ts`。
> - **個数のランダム化**：個数は難易度軸から除外し、各ラウンドで `countRange` プリセット（widе/half/few）からランダム抽選。本番教示は「**全て探せ**」（個数非表示）。`src/lib/v3/roundGen.ts`。
> - **チュートリアル Lv0**：初回プレイのみの固定3ラウンド（個数1/2/3・30秒・3x3・一方向・速度5、個数明示、誤答でも進む、進捗非干渉）。`src/screens/v3/TutorialScreen.tsx`、`UserProfile.tutorialCompleted` で初回判定。
> - **設定**：プレイ群に `repeatCount` ステッパー・`countRange` セグメントを追加。変数範囲設定から個数を除去、変化順は外側4変数のみ。`src/screens/v3/SettingsScreen.tsx`。
> - データモデル v3.2 再凍結（schemaVersion `3.2.0`）。i18n 全7ロケール対応。テスト **613 件**（55スイート）緑・`tsc` エラー0。ブラウザ（`serve`+Playwright）でオンボ→チュート→本番「全て探せ」→設定を検証済み。
