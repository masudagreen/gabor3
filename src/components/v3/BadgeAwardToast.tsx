/**
 * BadgeAwardToast.tsx — BG-2（components.md / sprint-9 screens.md S9-2、§6.4、v3.0）。
 *
 * ホーム結果カード（RC-1）上層で、バッジ獲得時に 1 度だけ流す演出。
 ...
 * - **一定時間で自動的に消える**（v3.1 改訂）：表示後 BADGE_HOLD_MS だけ保持してからフェードアウトし、
 *   アンマウントする（恒久表示しない）。reduced-motion 時もフェードなしで同じタイミングで消える。
 * - 音/ハプティクスは S10 で配線（onShown コールバックで発火点を用意。本コンポーネントは発火しない）。
 *
...
import { resolveBadgeNames } from '../../lib/v3/badgeView';

const BADGE_DURATION_MS = 1500;
/** 表示を保持する時間（フェードイン後）。これを過ぎると自動でフェードアウトして消える。 */
const BADGE_HOLD_MS = 2600;
/** フェードアウトの所要時間。 */
const BADGE_FADE_OUT_MS = 280;

export type BadgeAwardToastProps = {
  /** 今回新規獲得したバッジ ID（空なら何も表示しない）。 */
  badgeIds: readonly BadgeId[];
  /**
   * 演出表示開始時に 1 度だけ呼ばれる（S10：音/ハプティクス発火点）。
...
export const BadgeAwardToast: React.FC<BadgeAwardToastProps> = ({
  ...
  // 自動消滅後は描画しない（一定時間で消える、v3.1）。
  const [dismissed, setDismissed] = React.useState(false);
  const onShownRef = React.useRef(onShown);
  onShownRef.current = onShown;

  // 新しいバッジ集合が来たら再表示する（消滅状態をリセット）。
  React.useEffect(() => {
    if (hasBadges) setDismissed(false);
  }, [hasBadges, badgeIds]);
  ...
  React.useEffect(() => {
    ...
    // S10 発火点（音/ハプティクス）。表示開始時に 1 度だけ。
    onShownRef.current?.(badgeIds);

    // 一定時間保持してから自動でフェードアウト → アンマウント（恒久表示しない）。
    const scheduleDismiss = (reduced: boolean) => {
      timers.push(
        setTimeout(() => {
          if (cancelled) return;
          if (reduced) {
  ...
  if (!hasBadges || dismissed) return null;
