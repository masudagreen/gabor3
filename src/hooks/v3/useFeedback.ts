/**
 * useFeedback.ts — v3.0 音・ハプティクス発火の副作用配線（spec F-14、system §10）。
 *
 * 決定（lib/v3/feedback.ts の純関数）と再生（platform/audio.ts・haptics.ts の副作用）を
...
export type FeedbackEmitter = {
  ...
  emit: (event: FeedbackEventV3) => void;
};
...
export function useFeedback({
  ...
  const settingsRef = useRef<FeedbackSettingsV3>(settings);
  ...
  const emit = useCallback((event: FeedbackEventV3) => {
    // OS サイレントは native の audio session に委ねるため決定段階では false。
    const plan = decideFeedbackV3(event, settingsRef.current, false);
    if (plan.sound) {
