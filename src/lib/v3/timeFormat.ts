/**
 * timeFormat.ts — v3.1 改訂：プレイ時間（パッチを見ている時間）の表示整形（純関数）。
 *
 * セッション要約の「セッション時間」と履歴の「累計ゲーム時間」で共有する。
 * いずれも**パッチを見ている時間**（各ラウンドの実プレイ秒数の合計、3 秒開示は含めない）を
 * 人間可読な日本語に整形する。描画・i18n に依存しない（テスト容易性）。
 */
...
/**
 * セッション 1 回分のプレイ時間（パッチを見ている時間）を「M分S秒」で整形する。
 * 1 分未満は「S秒」。1 時間以上は「H時間M分」（秒は省略）。
 */
