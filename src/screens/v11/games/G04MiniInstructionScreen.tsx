/**
 * G04MiniInstructionScreen — S12-01（screens.md §2）。
 *
 * G-04 を初めてプレイするときに 1 度だけ表示するミニ説明画面。
 * 2 回目以降はスキップ（呼び出し元 AppRouterV11 が判断）。
 *
 * - 「じーっと見比べて、どちらの縞模様が濃いか」
 * - 4 つのリスト（60 秒 / 左右タップ / 何度でも変更可 / 錯視で差が消える注意）
 * - 「はじめる」Primary lg
 */
...
export type G04MiniInstructionScreenProps = {
  onStart: () => void;
  onBack: () => void;
};
...
export const G04MiniInstructionScreen: React.FC<
  G04MiniInstructionScreenProps
> = ({ onStart, onBack }) => {
  ...
  return (
    <View
      ...
      testID="g04-mini-instruction-screen"
    >
      <View style={styles.header}>
        <IconButton
          ...
          ariaLabel="一覧へ戻る"
          onPress={onBack}
          testId="g04-mini-instruction-back"
        />
        <Text
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View
          ...
          style={styles.list}
          testID="g04-mini-instruction-list"
        >
          {BULLETS.map((b, i) => (
        ...
        <View style={styles.cta}>
          <Button
            ...
            onPress={onStart}
            fullWidth
            testId="g04-mini-instruction-start"
          />
        </View>
      </ScrollView>
    </View>
