/**
 * G05MiniInstructionScreen — S13-01（screens.md §2）。
 *
 * G-05 を初めてプレイするときに 1 度だけ表示するミニ説明画面。
 * 2 回目以降はスキップ（呼び出し元 AppRouterV11 が判断）。
 *
 * - 「じーっと見比べて、どちらの縞が細かいか」
 * - 4 つのリスト（60 秒 / 左右タップ / 何度でも変更可 / cpd の差は微妙な注意）
 * - 「はじめる」Primary lg
 */
...
export type G05MiniInstructionScreenProps = {
  onStart: () => void;
  onBack: () => void;
};
...
const BULLETS = [
  ...
  '「左が細かい」「右が細かい」を選ぶ',
  '気が変われば何度でも変えてよい',
  '縞の太さの差は微妙でも、じっと見ると掴める',
];

export const G05MiniInstructionScreen: React.FC<
  G05MiniInstructionScreenProps
> = ({ onStart, onBack }) => {
  ...
  return (
    <View
      ...
      testID="g05-mini-instruction-screen"
    >
      <View style={styles.header}>
        <IconButton
          ...
          ariaLabel="一覧へ戻る"
          onPress={onBack}
          testId="g05-mini-instruction-back"
        />
        <Text
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View
          ...
          style={styles.list}
          testID="g05-mini-instruction-list"
        >
          {BULLETS.map((b, i) => (
        ...
        <View style={styles.cta}>
          <Button
            ...
            onPress={onStart}
            fullWidth
            testId="g05-mini-instruction-start"
          />
        </View>
      </ScrollView>
    </View>
