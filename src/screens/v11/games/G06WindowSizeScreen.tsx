/**
 * G06WindowSizeScreen — G-06 ガウス窓サイズ弁別（spec-v11.md §7.6、screens.md S21-G06-PLAY）。
 *
 * v1.1.2 Sprint 21 直接選択化（screens.md §6）：
 *   - horizontal-2 テキスト 2 択ボタン（旧 AnswerChoiceGroup）を**完全撤去**
 *   - 左右 2 ガボールパッチを ImageChoiceCell × 2 でラップして直接タップ回答
 *   - guidance：「より大きく広がっているパッチを選んでください」（18pt 以上）
 *   - data-target-id：`g06-left` / `g06-right`（resultMarks.ts と整合）
 *
 ...
 *   - 確定ボタンなし
 *   - 60 秒経過で自動採点
 *   - 未回答 = 不正解、staircase 易方向（SD 比を増やす方向 = max 方向）
 *
...
export type G06WindowSizeScreenProps = {
...
export const G06WindowSizeScreen: React.FC<G06WindowSizeScreenProps> = ({
  distanceCm,
  onAbort,
  onComplete,
  paused = false,
  ...
  const totalDurationMs = totalDurationMsForTest ?? GAME6_V11.totalDurationMs;

  const [staircase, setStaircase] = React.useState<StaircaseStateV11 | null>(
    null,
  );
  const [trial, setTrial] = React.useState<G06TrialSpec | null>(null);
  const [phase, setPhase] = React.useState<Phase>('loading');
  const [selectedSide, setSelectedSide] = React.useState<G06Side | null>(null);
  const [showAbort, setShowAbort] = React.useState(false);
  const [resultPayload, setResultPayload] =
    React.useState<G06TrialResult | null>(null);
  ...
  React.useEffect(() => {
    let cancelled = false;
    (async () => {
      const s = await loadStaircaseV11('G-06');
      if (cancelled) return;
      const t = buildG06Trial(s.currentParam, rng);
      setStaircase(s);
      setTrial(t);
      setPhase('playing');
    })();
  ...
  const trialRef = React.useRef<G06TrialSpec | null>(trial);
  const staircaseRef = React.useRef<StaircaseStateV11 | null>(staircase);
  const selectedSideRef = React.useRef<G06Side | null>(selectedSide);
  React.useEffect(() => {
  ...
  const handleTimeUp = React.useCallback(() => {
    ...
    const grading = gradeG06(t, selectedSideRef.current);
  ...
  // Sprint 20-B-3：result phase は同じ Screen 内に G06ResultScreen を重畳。
  if (phase === 'result' && resultPayload) {
    return (
      <View
        style={{ flex: 1, backgroundColor: colors.bgCanvas }}
        testID="g06-window-size-screen"
      >
        <G06ResultScreen
  ...
  const { patchSizePx, gapPx } = computeG06StimulusLayout({
    widthPx: vpW,
    heightPx: vpH,
  });
  ...
  return (
    ...
    >
      <GamePlaySurface
        ...
        onAbort={requestAbort}
        ariaInstruction="左右に並んだ 2 つの縞模様のうち、縞が見える範囲（縞模様の大きさ）が広いパッチを直接タップしてください。中心は同じくらい濃くても、ぼやけながら外側まで縞が伸びている方を選びます。気が変われば何度でも変更可。確定ボタンはありません。60 秒経過で自動採点します。"
        guidanceText="中央の + を見ながら、縞模様が大きく見えるのはどっち？"
        stimulusInteractive
        stimulusArea={
          <SideBySideStimulus
            left={trial.left}
            ...
            leftAriaLabel="左の縞模様（タップで「左の方が縞模様が大きい」と回答）"
            rightAriaLabel="右の縞模様（タップで「右の方が縞模様が大きい」と回答）"
            groupAriaLabel="左右の縞模様（縞模様が大きく見える方を選んでください）"
            leftDataTargetId="g06-left"
            rightDataTargetId="g06-right"
            cellTestIdPrefix="g06-stimulus"
            testId="g06-stimulus"
          />
        }
        answerChoices={null}
...
/** SD 比は小数 1 桁に丸める（gameRegistry.step=0.1 と整合） */
