/**
 * G05SfDiscrimScreen — F-07 G-05 受け入れテスト（spec-v11.md §7.5、screens.md S13-02）。
 */
...
describe('G05SfDiscrimScreen: F-07 / OPT-12', () => {
  ...
  it('Sprint 21：ガイド文「より細かい縞のパッチを選んでください」を表示', async () => {
    ...
    await findByTestId('g05-sf-discrim-screen');
    expect(queryAllByText(/より細かい縞のパッチを選んでください/).length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('Sprint 21：左右ガボールパッチ（g05-stimulus-left / right）が ImageChoiceCell でラップされる', async () => {
    const { findByTestId } = render(
      <G05SfDiscrimScreen
        distanceCm={40}
        onAbort={jest.fn()}
    ...
    await findByTestId('g05-sf-discrim-screen');
    await findByTestId('g05-stimulus-left');
    await findByTestId('g05-stimulus-right');
  });
  ...
  it('「左」ボタンを押下後 60 秒経過で正解側=left なら isCorrect=true', async () => {
    ...
    const leftBtn = await findByTestId('g05-stimulus-left');
  ...
  it('「右」ボタンタップ後に「左」を押すと最終回答が左に切り替わる', async () => {
    ...
    fireEvent.press(await findByTestId('g05-stimulus-right'));
    fireEvent.press(await findByTestId('g05-stimulus-left'));
    await act(async () => {
      jest.advanceTimersByTime(SHORT_DURATION + TICK_MS);
      await Promise.resolve();
    });
  ...
  it('staircase 1.2 初期（v1.1.4）→ 不正解で 1.25 方向（易、+step=0.05）に動く', async () => {
    // 過去の永続化なし：初期 currentParam=1.2（gameRegistry より、v1.1.4 難化）
    const onComplete = jest.fn();
    const { findByTestId } = render(
      <G05SfDiscrimScreen
        distanceCm={40}
    ...
    await waitFor(() => expect(onComplete).toHaveBeenCalledTimes(1));
    const result = onComplete.mock.calls[0][0];
    expect(result.playedParam).toBeCloseTo(1.2, 5); // initial（v1.1.4）
    expect(result.isCorrectForStaircase).toBe(false);
    // staircase が易方向（max 方向 +1 step=0.05）に動いた状態が AsyncStorage に保存されている
    const raw = await AsyncStorage.getItem('gaboreye:v1.1:staircase:G-05');
    expect(raw).not.toBeNull();
    const parsed = JSON.parse(raw as string);
    // 1.2 + 0.05 = 1.25（浮動小数誤差は toBeCloseTo で許容）
    expect(parsed.currentParam).toBeCloseTo(1.25, 5);
  });

  it('閾値（thresholdRatio）は小数 1 桁の数値（例 1.5）', async () => {
    const onComplete = jest.fn();
    render(
      <G05SfDiscrimScreen
        distanceCm={40}
  ...
  it('staircase が max=2.0 上限を超えない（連続不正解しても max でクランプ）', async () => {
    // 既存 staircase を currentParam=2.0 で保存しておく
    const existing = {
      gameId: 'G-05',
      currentParam: 2.0,
    ...
    fireEvent.press(await findByTestId('g05-stimulus-right')); // 不正解
    await act(async () => {
      jest.advanceTimersByTime(SHORT_DURATION + TICK_MS);
      await Promise.resolve();
    });
